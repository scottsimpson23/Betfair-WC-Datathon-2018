...
#Read data
df <- read_csv("wc_datathon_dataset_rolling.csv")
...
tic.clearlog()
tic("create country specific columns for outcome and for/against")
...
for(numb in c(2,5,10,20,50)) {
  ...
  #remove the WC removal code for this version
  # for(cn in col_list){
  #   
...
for(r in seq(1,n,by=1)) {
    ...
    elotable$diffrating[r] <- if_else(elotable$is_team_1_home[r] == 1,
...
#split
df_test <- df %>% slice(14968:14971)
df_test %>% select(team_1, team_2) 
df <- df %>% slice(1:14967)

df_train_test %>% select(team_1, team_2) %>% tail
...
#load tunings & set
load("tuned_XGB_params_inplay.RData")
lrn.xgb = setHyperPars(lrn.xgb, par.vals = xg_tr$x)

load("tuned_RF_params_inplay.RData")
lrn.rf = setHyperPars(lrn.rf, par.vals = rf_tr$x)

load("tuned_h2odeep_params_inplay.RData")
...
#stacked learner
tic("train the learner & make predictions")
#train stacked
h2o.init(nthreads = parallel::detectCores()-1)
...
prediction_df %>% write_csv("prediction_20180704_stacked.csv")
h2o.shutdown(prompt = FALSE)
toc(log=TRUE)
...
```{r Train the other learners if needed}
#train XGB
#h2o.init(nthreads = parallel::detectCores()-1)
...
save(xg_tr, file="tuned_XGB_params_inplay.RData")
...
lrn.rf = setHyperPars(lrn.rf, par.vals = rf_tr$x)
save(rf_tr, file="tuned_RF_params_inplay.RData")
...
save(h2odeep_tr, file="tuned_h2odeep_params_inplay.RData")
lrn.h2odeep = setHyperPars(lrn.h2odeep, par.vals = h2odeep_tr$x)
...
prediction_df <- tibble(df_test$team_1, df_test$team_2, win_prob, draw_prob, lose_prob)
prediction_df %>% write_csv("prediction_20180618_stacked.csv")
h2o.shutdown(prompt = FALSE)
...
>>>>>>> 329782a9d2d3f121c3f9455558c935da99f1c1fa
