...
prediction_stacked <- predict(model_stacked, newdata = df_test)
prediction_stacked
...
h2ogbm_ps <- makeParamSet(
makeIntegerParam("ntrees", lower=1,upper=500),
makeDiscreteParam("max_depth", seq(2, 24, by = 2)),
makeDiscreteParam("min_rows",seq(10, 150, by = 10)),
#makeDiscreteParam("sample_rate", seq(0.1, 1, by = 0.1)),
makeDiscreteParam("nbins", round(2 ^ seq(2, 6, length = 15)))
)

h2ogbm.mc <- mlrMBO::makeMBOControl(save.on.disk.at = 1:100 )

h2o.init(nthreads = parallel::detectCores()-1)
h2ogbm_tc = makeTuneControlMBO(budget=100, mbo.control = h2ogbm.mc)
h2ogbm_tr = tuneParams(learner = lrn.h2ogbm, task = task, resampling = rin, measures = logloss, control=h2ogbm_tc, par.set = h2ogbm_ps)
h2o.shutdown(prompt=FALSE)

save(h2ogbm_tr, file="tuned_h2ogbm_params.RData")
lrn.h2ogbm = setHyperPars(lrn.h2ogbm, par.vals = h2ogbm_tr$x)
