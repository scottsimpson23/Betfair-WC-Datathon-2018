...
df$outcome <- parse_factor(df$outcome, levels = c("lose", "draw", "win"))
levels(df$outcome)
...
save(df, file = "full_dataset_for_learning.Rdata")
...
```{r Split the dataset into train and test}

#backup full dataset
df_train_test <- df

#split
df_test <- df %>% slice(14912:14959)
df <- df %>% slice(1:14911)
...
lrn.h2odeep = makeLearner("classif.h2o.deeplearning", predict.type="prob")

lrn.h2orf = makeLearner("classif.h2o.randomForest", predict.type="prob")
...
lrn.stack = makeStackedLearner(base.learners, super.learner = NULL, predict.type = "prob",
  method = "average", use.feat = FALSE, resampling = NULL,
  parset = list(metric = measureLogloss))
...
h2o.init(nthreads = parallel::detectCores()-1)
resample.h2odeep = resample(learner=lrn.h2odeep, task=task, resampling = rin, measures = logloss)
h2o.shutdown(prompt=FALSE)

h2o.init(nthreads = parallel::detectCores()-1)
resample.h2orf = resample(learner=lrn.h2orf, task=task, resampling = rin, measures = logloss)
h2o.shutdown(prompt=FALSE)


#parallelStartSocket(parallel::detectCores()-1)
h2o.init(nthreads = parallel::detectCores()-1)
resample.stack = resample(learner=lrn.stack, task=task, resampling = rin, measures = logloss)
h2o.shutdown(prompt=FALSE)
#parallelStop()
...
perfmeasures.h2odeep <- performance(resample.h2odeep$pred, measures = list(mmce, acc, logloss))
perfmeasures.h2odeep
calculateConfusionMatrix(resample.h2odeep$pred)


perfmeasures.h2orf <- performance(resample.h2orf$pred, measures = list(mmce, acc, logloss))
perfmeasures.h2orf
calculateConfusionMatrix(resample.h2orf$pred)

perfmeasures.stack <- performance(resample.stack$pred, measures = list(mmce, acc, logloss))
perfmeasures.stack
calculateConfusionMatrix(resample.stack$pred)

```


```{r Train the learner & make predictions}

#setup output task
task_predict = makeClassifTask(id = "football", data = df_test, target = "outcome")
task_predict = dropFeatures(task = task_predict, c("team_1", "team_2", "team_1_goals", "team_2_goals", "team_1_outcome", "team_2_outcome", "team_1_index", "team_2_index"))

#stacked learner

#train stacked
h2o.init(nthreads = parallel::detectCores()-1)
model_stacked <- train(lrn.stack, task)

#model_stacked

#predict & save, 100 rep bagging for the final probs

#prediction_stacked_1 <- prediction_stacked
prediction_stacked <- predict(model_stacked, task = task_predict)
pred_stack = list()
win_prob <- prediction_stacked$data$prob.win
draw_prob <- prediction_stacked$data$prob.draw
lose_prob <- prediction_stacked$data$prob.lose
win_prob <- 0
draw_prob <- 0
lose_prob <- 0

for(i in 1:100) {
  pred_stack[[i]] <- predict(model_stacked, task = task_predict)
  win_prob <- win_prob + pred_stack[[i]]$data$prob.win
  lose_prob <- lose_prob + pred_stack[[i]]$data$prob.lose
  draw_prob <- draw_prob + pred_stack[[i]]$data$prob.draw
}
win_prob <- win_prob / 100
draw_prob <- draw_prob / 100
lose_prob <- lose_prob / 100

prediction_stacked

prediction_df <- tibble(df_test$team_1, df_test$team_2, win_prob, draw_prob, lose_prob)
prediction_df %>% write_csv("prediction_3_stacked.csv")
h2o.shutdown(prompt = FALSE)

#train XGB
#h2o.init(nthreads = parallel::detectCores()-1)
model_xgb <- train(lrn.xgb, task)
#h2o.shutdown(prompt = FALSE)
model_xgb

#predict & save
prediction_xgb1 <- prediction_xgb
prediction_xgb <- predict(model_xgb, task = task_predict)
prediction_xgb

prediction_df <- tibble(df_test$team_1, df_test$team_2, prediction_xgb$data$prob.win, prediction_xgb$data$prob.draw, prediction_xgb$data$prob.lose)
prediction_df %>% write_csv("prediction_xgb.csv")


#train rf
#h2o.init(nthreads = parallel::detectCores()-1)
model_rf <- train(lrn.rf, task)
#h2o.shutdown(prompt = FALSE)
model_rf

#predict & save
prediction_rf1 <- prediction_rf
prediction_rf <- predict(model_rf, task = task_predict)
prediction_rf

prediction_df <- tibble(df_test$team_1, df_test$team_2, prediction_rf$data$prob.win, prediction_rf$data$prob.draw, prediction_rf$data$prob.lose)
prediction_df %>% write_csv("prediction_rf.csv")

#train h2odeep
h2o.init(nthreads = parallel::detectCores()-1)
model_h2odeep <- train(lrn.h2odeep, task)

model_h2odeep

#predict & save
prediction_h2odeep1 <- prediction_h2odeep
prediction_h2odeep <- predict(model_h2odeep, task = task_predict)

prediction_h2odeep

h2o.shutdown(prompt = FALSE)
prediction_df <- tibble(df_test$team_1, df_test$team_2, prediction_h2odeep$data$prob.win, prediction_h2odeep$data$prob.draw, prediction_h2odeep$data$prob.lose)
prediction_df %>% write_csv("prediction_h2odeep.csv")
...
#lrn.xgb1 = setHyperPars(lrn.xgb1, par.vals = xg_tr$x)
#lrn.xgb2 = setHyperPars(lrn.xgb2, par.vals = xg_tr$x)
#lrn.xgb3 = setHyperPars(lrn.xgb3, par.vals = xg_tr$x)
#lrn.xgb4 = setHyperPars(lrn.xgb4, par.vals = xg_tr$x)
...
h2odeep_ps <- makeParamSet(
makeDiscreteParam(id = "activation", values = c("Rectifier", "Maxout", "Tanh", "RectifierWithDropout", "MaxoutWithDropout", "TanhWithDropout")),
makeDiscreteParam(id = "hidden", values = list(a = c(10), b = c(30), c = c(40), d = c(50), e = c(100), f = c(200), g= c(50,50), h = c(10,10))), 
makeNumericParam("l1", lower = 0, upper = 1e-3),
makeNumericParam("l2", lower = 0, upper = 1e-3)
) 

#parallelStartSocket(parallel::detectCores()-1)
h2o.init()
h2odeep_tc = makeTuneControlMBO(budget=100)
h2odeep_tr = tuneParams(learner = lrn.h2odeep, task = task, resampling = rin, measures = logloss, control=h2odeep_tc, par.set = h2odeep_ps)
#h2o.shutdown(prompt=FALSE)
#parallelStop()

save(h2odeep_tr, file="tuned_h2odeep_params.RData")
lrn.h2odeep = setHyperPars(lrn.h2odeep, par.vals = h2odeep_tr$x)


#h2o rf
getParamSet("classif.h2o.randomForest")

h2orf_ps <- makeParamSet(
makeIntegerParam("ntrees",lower = 20, upper = 200),
makeIntegerParam("mtries", lower = 3, upper = 10),
makeIntegerParam("max_depth", lower = 10, upper = 50)
)

h2orf.mc <- mlrMBO::makeMBOControl(save.on.disk.at = 1:100 )

#parallelStartSocket(parallel::detectCores()-1)
h2o.init(nthreads = 4)
h2orf_tc = makeTuneControlMBO(budget = 100)
h2orf_tr = tuneParams(learner = lrn.h2orf, task = task, resampling = rin, measures = logloss, control=h2orf_tc, par.set = h2orf_ps)
h2o.shutdown(prompt=FALSE)

save(h2orf_tr, file="tuned_h2orf_params.RData")
lrn.h2orf = setHyperPars(lrn.h2orf, par.vals = h2orf_tr$x)
...
svm_ps <- makeParamSet(
makeNumericParam("cost", lower = 0.001, upper = 1000),
makeNumericParam("gamma", lower = 0.5, upper = 4)
) 

#parallelStartSocket(parallel::detectCores()-1)
parallelStartSocket(4)
svm_tc = makeTuneControlGenSA(budget=100)
svm_tr = tuneParams(learner = lrn.svm, task = task, resampling = rin, measures = logloss, control=svm_tc, par.set = svm_ps)
parallelStop()

lrn.svm = setHyperPars(lrn.svm, par.vals = svm_tr$x)
save(svm_tr, file="tuned_svm_params.RData")
...
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "h2o deeplearning default untuned", mmce = 0.5585938, accuracy = 0.4414062, logloss = 1.1757131)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "h2o gbm default untuned", mmce = 0.4648438, accuracy = 0.5351562, logloss = 1.0230928)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "h2o deeplearning tuned 1 round only", mmce = 0.4453125, accuracy = 0.5546875, logloss = 1.0356291)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "h2o gbm tuned 1 round", mmce = 0.4843750, accuracy = 0.5156250, logloss = 0.9867435)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "stacked rf and xgb hill climb", mmce = 0.4296875, accuracy = 0.5703125, logloss = 0.9723495)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "stacked rf and xgb average", mmce = 0.4218750, accuracy = 0.5781250, logloss = 0.9644549)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "h2o deeplearning tuned", mmce = 0.4218750, accuracy = 0.5781250, logloss = 0.9737596)
logtablewc <- logtablewc %>% add_row(comments = "full set with ELO ratings, data to 20180609", CV = "4 x world cup rounds only, growing window", learner = "stacked rf, xgb, h2odeep average", mmce = 0.4375000, accuracy = 0.5625000, logloss = 0.9615743)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "tuned xgboost", mmce = 0.4257812, accuracy = 0.5742188, logloss = 0.9730688)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "tuned randomforest", mmce = 0.4375000, accuracy = 0.5625000, logloss = 0.9659107)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "tuned h2odeep", mmce = 0.4296875, accuracy = 0.5703125, logloss = 0.9708991)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "stacked rf, xgb, h2odeep average", mmce = 0.4218750, accuracy = 0.5781250, logloss = 0.9599997)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "svm untuned", mmce = 0.5546875, accuracy = 0.4453125, logloss = 1.0716269)
logtablewc <- logtablewc %>% add_row(comments = "final set", CV = "4 x world cup rounds only, growing window", learner = "h2orf untuned", mmce = 0.4570312, accuracy = 0.5429688, logloss = 0.9952351)
...
```{r Check the submissions against actuals - calculate logloss}

#read spreadsheet tab

results = read.xlsx("Submission Assembly.xlsx", sheet = "Tracking", detectDates = TRUE)
...
results$outcome = parse_factor(results$outcome, levels = c("win", "draw", "lose"))


scores <- tibble(submission = 1:5, score = 0)

scores$score[[1]] <- measureLogloss(as.matrix(tibble(win = results$sub1_win, draw = results$sub1_draw, lose = results$sub1_lose)), results$sub1_outcome)
scores$score[[2]] <- measureLogloss(as.matrix(tibble(win = results$sub2_win, draw = results$sub2_draw, lose = results$sub2_lose)), results$sub2_outcome)
scores$score[[3]] <- measureLogloss(as.matrix(tibble(win = results$sub3_win, draw = results$sub3_draw, lose = results$sub3_lose)), results$sub3_outcome)
scores$score[[4]] <- measureLogloss(as.matrix(tibble(win = results$sub4_win, draw = results$sub4_draw, lose = results$sub4_lose)), results$sub4_outcome)
scores$score[[5]] <- measureLogloss(as.matrix(tibble(win = results$sub5_win, draw = results$sub5_draw, lose = results$sub5_lose)), results$sub5_outcome)
