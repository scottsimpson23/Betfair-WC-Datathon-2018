...
for(numb in c(2,5,10,20,50)) {
  ...
  #grab each set of metrics (outcomes, goals, opponentgoals) and sum each row to generate the match specific columns
  
  temp_out_frame <- as.tibble(temp_out_frame)

  staging_frame <- temp_out_frame %>% select(ends_with(paste(numb,"_rolling_outcome_stage", sep=""))) %>%
